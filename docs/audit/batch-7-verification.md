...
| Area | Command | Result |
| --- | --- | --- |
| Recommendation API and core | `mvn -B -pl backend/recommendation-service -am -Dtest=RecommendationControllerTest,RecommendationRepositoryTest,RecommendationApplicationServiceTest -Dsurefire.failIfNoSpecifiedTests=false test` | 13 tests passed |
...
| Full Maven reactor | `mvn -B test` | 113 tests passed, 0 failures, 0 errors |
| Frontend unit suite | `npm test -- --watch=false` from `frontend` | 83 tests passed |
...
- The Angular recommendation client preserves server ranking and reason text, and the page supports URL-backed filters, loading, empty/cold-start, validation, outage, retry, browse fallback, details, and watchlist-add states.
